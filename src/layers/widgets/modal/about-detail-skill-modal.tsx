...
const ContentMapper = (type: SkillSet) => {
  switch (type) {
    ...
    case 'react':
      return (
        <ul className="list-disc">
          ...
          <li>사내 프론트엔드 프로젝트를 모두 react, nextjs로 개발하였음.</li>
          <li>
    ...
    case 'html-js-css':
      return (
        <ul className="list-disc">
          <li>구현에 필요한 기능에 대해 mdn을 참고하여 개발할 수 있음.</li>
          <li>
            정보 공유 등 간단한 기능 시현을 위한 기본적인 예제들(canvas, design
            pattern 등)은 바닐라 html, js, css로 구현하며 기초 기술에 대한
            감각을 유지하려 노력함.
          </li>
          <li>
            배열 프로토타입 메서드를 직접 구현하거나, 모듈 js의 console.log 호출
            순서 등의 기초적인 자바스크립트 탐구 활동을 통해 보다 탄탄한 js 기초
            지식을 가지고 있음.
          </li>
        </ul>
      )
